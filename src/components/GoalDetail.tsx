...
import { CommentFields, GoalFields, ProgressUpdateFields } from "@/types/move";
import {
    CalendarDays,
    Clock,
    Coins,
...
export function GoalDetail({ id }: GoalDetailProps) {
    ...
    // 从API获取goalAgentId
    const { data: goalAgentId, refetch: refetchAgentId } = useGetGoalAgentId({
        goalId: id,
    });
    ...
    const { data: progressUpdates, refetch: refetchProgressUpdates } =
        useGetGoalProgressUpdates({ goalId: id });
    ...
    useEffect(() => {
        if (data && address) {
            const goalData = (data as GoalFields).fields;
            const isConfirmed =
                                                            ...
                                                            <button
                                                                onClick={async () => {
                                                                    try {
                                                                        ...
                                                                        let filename = `proof-${update.fields.proof_file_blob_id}`;
                                                                        if (
                                                                            contentDisposition
                                                                        ) {
                                                                            const matches =
