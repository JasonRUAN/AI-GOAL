module ai_goal::ai_goal {
    ...
    public entry fun create_goal(
        ...
        let paid = coin::split(payment_coin, amount, ctx);
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_CREATE_GOAL,
            AIRDROP_METHOD_CREATE_GOAL,
            ctx
        );
    }
    ...
    public entry fun confirm_witness(
        ...
        let sender = ctx.sender();
        assert!(
            vector::contains(&goal.witnesses, &sender),
            ENotWitness
        );
        assert!(
        ...
        event::emit(
            EventWitnessConfirmed {goal_id, witness: sender,}
        );

        airdrop(
            vault,
            AIRDROP_AMOUNT_CONFIRM_WITNESS,
            AIRDROP_METHOD_CONFIRM_WITNESS,
            ctx
        );
    }
    ...
    public entry fun complete_goal(
        ...
        // 添加到已完成目标列表
        vector::push_back(
            &mut manager.completed_goals,
            goal_id
        );

        event::emit(
            EventGoalCompleted {goal_id, completer: sender,}
        );
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_COMPLETE_GOAL,
            AIRDROP_METHOD_COMPLETE_GOAL,
            ctx
        );
    }
    ...
    public entry fun fail_goal(
        ...
        assert!(
            clock::timestamp_ms(clock) >= goal.deadline,
            EDeadlineNotReached
        );
        ...
        assert!(
            !vector::contains(&goal.witnesses, &sender) && !vector::contains(
                &goal.confirmations, &sender
            ) && goal.creator != sender,
            ENotGoalRelatedMember
        );
        ...
        event::emit(
            EventGoalFailed {goal_id, failer: sender,}
        );
        ...
        while (i < witness_count) {
            let per_witness_coin = coin::take(
                &mut manager.balance,
                per_witness_amount,
                ctx
            );
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_FAIL_GOAL,
            AIRDROP_METHOD_FAIL_GOAL,
            ctx
        );
    }
    ...
    public fun create_agent(
        ...
        assert!(
            agent_id.length() == AGENT_ID_LENGTH,
            EAgentIdTooShort
        );
        assert!(
            agent_name.length() >= MIN_LENGTH,
            EAgentNameTooShort
        );
        assert!(
            charactor_json.length() >= CHARACTOR_JSON_MIN_LENGTH,
            ECharactorJsonTooShort
        );
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_CREATE_AGENT,
            AIRDROP_METHOD_CREATE_AGENT,
            ctx
        );
    }
    ...
    public fun update_agent(
        ...
        let agent = dynamic_object_field::borrow_mut<String, Agent>(
            &mut manager.id, *agent_id
        );
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_UPDATE_AGENT,
            AIRDROP_METHOD_UPDATE_AGENT,
            ctx
        );
    }
    ...
    public entry fun create_comment(
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_CREATE_COMMENT,
            AIRDROP_METHOD_CREATE_COMMENT,
            ctx
        );
    }
    ...
    public entry fun update_progress(
        ...
        // 确保进度百分比有效（0-100）
        assert!(
            progress_percentage > 0 && progress_percentage >= goal.progress_percentage &&
                progress_percentage <= 100,
            EInvalidProgressPercentage
        );
        ...
        airdrop(
            vault,
            AIRDROP_AMOUNT_UPDATE_PROGRESS,
            AIRDROP_METHOD_UPDATE_PROGRESS,
            ctx
        );
    }
}
